...
func (s *Suite) TestGetAndViewWhenKeyNotPresent(t *testing.T) {
	...
	err = bs.View(context.Background(), c, func(bytes []byte) error {
...
func (s *Suite) TestGetAndViewWhenKeyIsNil(t *testing.T) {
	...
	err = bs.View(context.Background(), cid.Undef, func(bytes []byte) error {
		t.FailNow() // should not be invoked
		return nil
	})
	require.Equal(t, ipld.ErrNotFound{Cid: cid.Undef}, err)
...
func (s *Suite) TestPutThenGetAndViewBlock(t *testing.T) {
	...
	err = bs.View(context.Background(), orig.Cid(), func(bytes []byte) error {
...
func (s *Suite) TestInsertExistingBlock(t *testing.T) {
	...
	var err error
	err = bs.Put(context.Background(), orig1)
	require.NoError(t, err)

	err = bs.Put(context.Background(), orig1)
	require.NoError(t, err)

	err = bs.Put(context.Background(), orig2)
	require.NoError(t, err)

	err = bs.Put(context.Background(), orig2)
	require.NoError(t, err)

	err = bs.PutMany(context.Background(), []blocks.Block{orig1, orig2})
	require.NoError(t, err)
}
...
func (s *Suite) TestCidv0v1(t *testing.T) {
	...
	err = bs.View(context.Background(), cid.NewCidV1(cid.DagProtobuf, orig.Cid().Hash()), func(bytes []byte) error {
		require.Equal(t, orig.RawData(), bytes)
		return nil
	})
	require.NoError(t, err)
...
func (s *Suite) TestPutThenGetSizeBlock(t *testing.T) {
	...
	emptySize, err := bs.GetSize(context.Background(), emptyBlock.Cid())
	require.NoError(t, err)
...
func (s *Suite) TestPutMany(t *testing.T) {
	...
	for _, blk := range blks {
		...
		err = bs.View(context.Background(), blk.Cid(), func(bytes []byte) error {
